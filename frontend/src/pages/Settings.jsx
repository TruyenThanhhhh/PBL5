  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Chuyển về profile sau 1.5s
        setTimeout(() => navigate('/profile'), 1500);
      } else {
        setProfileMessage(result.message || t.saveError);
      }
    } catch (error) {
      setProfileMessage(t.saveError);
    } finally {
    ...
    <div className="min-h-screen bg-[#f8f9fa] font-sans text-gray-900 pb-12">
      ...
      <header className="flex items-center justify-between py-3 px-6 bg-white sticky top-0 z-50 border-b border-gray-100 shadow-sm">
        ...
        <div className="flex-1 flex justify-center">
          <nav className="hidden md:flex items-center gap-8 text-[14px] font-bold text-gray-500">
            <button onClick={() => navigate('/dashboard')} className="hover:text-gray-900 transition-colors py-4">{t.navDashboard}</button>
            <button onClick={() => navigate('/explore')} className="hover:text-gray-900 transition-colors py-4">{t.navExplore}</button>
            <button onClick={() => navigate('/community')} className="hover:text-gray-900 transition-colors py-4">{t.navCommunity}</button>
          </nav>
        </div>
      ...
      <main className="max-w-[1320px] mx-auto px-6 2xl:px-8 pt-10 flex gap-8 items-start">
          ...
          <nav className="space-y-1 text-[13px] font-bold text-gray-600 mb-8">
            {menuItems.map(({ id, label, icon: Icon }) => (
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-left ${activeTab === id ? 'bg-white text-[#f44336] shadow-sm' : 'hover:bg-white hover:shadow-sm'}`}
              >
        ...
        <section className="flex-1 min-w-0">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                  <div className="relative mb-16 rounded-xl">
                    <label className="block w-full h-32 bg-gray-200 relative overflow-hidden rounded-2xl group cursor-pointer border border-gray-100">
                      <img
                  ...
                  <div className="pt-2">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 ml-1">{t.displayName}</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 ml-1">{t.bio}</label>
                        <input
                          type="text"
                    ...
                    {profileMessage && (
                      <div className={`mb-6 px-4 py-3 rounded-xl flex items-center gap-3 text-[13px] font-bold animate-in fade-in ${profileMessage === t.saveSuccess ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-600 border border-red-100'}`}>
                        {profileMessage === t.saveSuccess ? <CheckCircle size={18}/> : <AlertCircle size={18}/>}
                        {profileMessage}
                      </div>
                    )}
          ...
          {/* PLACEHOLDER CHO CÁC TÍNH NĂNG CHƯA HOÀN THIỆN */}

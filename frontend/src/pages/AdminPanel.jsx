...
import {
  ...
  MapPin, Star, Calendar, MoreVertical, X, AlertTriangle, Layers
} from 'lucide-react';
...
/* STREAMING_CHUNK:Từ điển dịch ngôn ngữ (Đã thêm phần Community)... */
const copy = {
  vi: {
    cancel: 'Hủy',
    confirm: 'Xác nhận',
    ...
    communitiesNav: 'Cộng đồng',
    manageCommunities: 'Quản lý Cộng đồng',
    manageCommunitiesDesc: 'Kiểm soát, theo dõi và quản lý các cộng đồng do User tạo.',
    totalCommunities: 'Tổng Cộng đồng',
    searchCommunities: 'Tìm theo tên, mô tả...',
    noCommunitiesFound: 'Không tìm thấy cộng đồng nào',
    communityName: 'Cộng đồng',
    communityMembers: 'Thành viên',
    communityCreator: 'Người tạo',
    deleteCommunity: 'Xóa Cộng đồng',
    deleteCommunityTitle: 'Xóa Cộng đồng?',
    cannotLoadCommunities: 'Lỗi tải dữ liệu cộng đồng',
    communityDeleted: 'Đã xóa cộng đồng',
    deleteCommunityError: 'Lỗi khi xóa cộng đồng',
  },
  en: {
    cancel: 'Cancel',
    confirm: 'Confirm',
    ...
    communitiesNav: 'Communities',
    manageCommunities: 'Manage Communities',
    manageCommunitiesDesc: 'Monitor and manage communities created by users.',
    totalCommunities: 'Total Communities',
    searchCommunities: 'Search by name, desc...',
    ...
    cannotLoadCommunities: 'Failed to load communities',
    communityDeleted: 'Community deleted',
    deleteCommunityError: 'Failed to delete community',
  },
};

/* STREAMING_CHUNK:Components Modal & Toast... */
...
/* STREAMING_CHUNK:CommunitiesTab (Tab quản lý Community mới)... */
function CommunitiesTab({ showToast, t, locale }) {
  ...
  const fetchCommunities = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/communities/all`, { headers: authHeader() });
      if (!res.ok) throw new Error();
      const data = await res.json();
      setCommunities(Array.isArray(data) ? data : []);
    } catch {
      showToast('error', t.cannotLoadCommunities);
    } finally {
      setLoading(false);
    }
  }, [showToast, t.cannotLoadCommunities]);

  useEffect(() => { fetchCommunities(); }, [fetchCommunities]);

  const handleDelete = async (communityId) => {
    try {
      const res = await fetch(`${API}/communities/${communityId}`, { method: 'DELETE', headers: authHeader() });
      if (!res.ok) throw new Error();
      setCommunities(prev => prev.filter(c => c._id !== communityId));
      showToast('success', t.communityDeleted);
    } catch {
      showToast('error', t.deleteCommunityError);
    }
    setConfirm(null);
  };
  ...
  const filtered = communities.filter(c => {
    ...
    return c.name?.toLowerCase().includes(keyword) || c.description?.toLowerCase().includes(keyword);
  });
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">{t.totalCommunities}</p>
          <p className="text-2xl font-black text-indigo-600">{communities.length}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm mb-4 p-4 flex flex-wrap gap-3 items-center justify-between">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-[#f4f4f5] rounded-xl text-[13px] font-medium focus:outline-none focus:ring-2 focus:ring-indigo-200"
          />
        </div>
        <button onClick={fetchCommunities} className="flex items-center gap-2 text-[13px] font-bold text-gray-500 hover:text-gray-900 px-3 py-2 rounded-xl hover:bg-gray-100 transition-colors">
          <RefreshCw size={14} /> {t.reload}
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="text-[13px] font-medium">{t.noCommunitiesFound}</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-gray-100 bg-[#fafafa]">
                  <th className="text-left px-5 py-3 font-black text-gray-400 text-[11px] uppercase tracking-wider">{t.communityName}</th>
                  <th className="text-left px-4 py-3 font-black text-gray-400 text-[11px] uppercase tracking-wider hidden md:table-cell">{t.communityCreator}</th>
                  <th className="text-left px-4 py-3 font-black text-gray-400 text-[11px] uppercase tracking-wider">{t.communityMembers}</th>
                  <th className="text-left px-4 py-3 font-black text-gray-400 text-[11px] uppercase tracking-wider hidden lg:table-cell">{t.date}</th>
                  <th className="px-4 py-3"></th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(c => (
                  <tr key={c._id} className="border-b border-gray-50 hover:bg-[#fafafa] transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        {c.banner ? (
                          <img src={c.banner} alt="" className="w-10 h-10 rounded-lg object-cover flex-shrink-0 border border-gray-100" />
                        ) : (
                          <div className="w-10 h-10 rounded-lg bg-indigo-50 flex items-center justify-center flex-shrink-0 text-indigo-500">
                            <Layers size={16} />
                          </div>
                        )}
                        <div>
                          <p className="font-bold text-gray-900 line-clamp-1 max-w-[200px]">{c.name}</p>
                          <p className="text-[11px] text-gray-400 line-clamp-1 max-w-[200px]">{c.description || '—'}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3.5 hidden md:table-cell">
                      <div className="flex items-center gap-2">
                         <div className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-black text-gray-500 flex-shrink-0">
                           {c.createdBy?.username?.[0]?.toUpperCase() || '?'}
                         </div>
                         <span className="font-medium text-gray-700">{c.createdBy?.username || t.anonymous}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3.5 text-gray-700 font-bold">
                      {c.members?.length || 0}
                    </td>
                    <td className="px-4 py-3.5 text-gray-500 hidden lg:table-cell">{fmt(c.createdAt, locale)}</td>
                    <td className="px-4 py-3.5 text-right">
                      <button
                        onClick={() => setConfirm({ id: c._id, title: t.deleteCommunityTitle, desc: `"${c.name}" ${t.permanentDelete}` })}
...
/* STREAMING_CHUNK:DashboardTab và AdminPanel chính... */
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <button onClick={() => onOpenTab('posts')} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:border-[#f44336]/30 hover:shadow-md transition-all text-left group">
        ...
        <button onClick={() => onOpenTab('users')} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:border-blue-500/30 hover:shadow-md transition-all text-left group">
          <Users size={24} className="text-blue-500 mb-3"/>
          <h3 className="text-[15px] font-black text-gray-900 mb-1 group-hover:text-blue-600 transition-colors">{t.manageUsers}</h3>
          <p className="text-[12px] text-gray-500">{t.manageUsersDesc}</p>
        </button>
        <button onClick={() => onOpenTab('communities')} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:border-indigo-500/30 hover:shadow-md transition-all text-left group">
          <Layers size={24} className="text-indigo-500 mb-3"/>
          <h3 className="text-[15px] font-black text-gray-900 mb-1 group-hover:text-indigo-600 transition-colors">{t.manageCommunities}</h3>
          <p className="text-[12px] text-gray-500">{t.manageCommunitiesDesc}</p>
        </button>
      </div>
    </div>
  );
...
export default function AdminPanel() {
  ...
  const navItems = [
    ...
    { id: 'communities', icon: Layers, label: t.communitiesNav }, // THÊM MENU COMMUNITY VÀO ĐÂY
  ];
    ...
    <div className="flex h-screen bg-[#f8f9fa] font-sans text-gray-900 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="h-[64px] bg-white border-b border-gray-100 flex items-center justify-between px-8 flex-shrink-0">
          <div>
            <h2 className="text-[16px] font-black text-gray-900">
              ...
              {activeTab === 'communities' && t.manageCommunities}
        ...
        <div className="flex-1 overflow-y-auto p-8">
          ...
          {activeTab === 'communities' && <CommunitiesTab showToast={showToast} t={t} locale={locale} />}
          {activeTab === 'profile' && (
